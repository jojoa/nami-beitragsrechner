...
class NamiAccounting:
    ...
    def process(self):
        ...
        # print(tabulate2x(result))
        list_of_members_non_debit = []
        # Error variables
        list_of_missing_mandat = []
        list_of_corrupt_activities = []
        list_of_members_second_half_of_year = []
        # Liste von Mitgliedern die erst im neuen Jahr dazu gekommen sind.
        list_of_members_in_new_year = []
        ...
        for r in result:
            ...
            if member.kontoverbindung.zahlungsKonditionId != 1:
                list_of_members_non_debit.append(member)
                tmp = len(dpsg_members)
                dpsg_members = [
                    x for x in dpsg_members if not x.compare(member)]
                continue
            ...
            try:
                ...
                tools.print_error(
                    'IBAN nicht korrekt für Mitglied ' + combined_name + ' Error: ' + str(e))
                raise ValueError('IBAN nicht korrekt für Mitglied ' +
                                 combined_name + ' Error: ' + str(e))

            try:
                BIC(member.kontoverbindung.bic)
            except ValueError as e:
                tools.print_error(
                    'BIC nicht korrekt für Mitglied ' + combined_name + ' Error: ' + str(e))
                raise ValueError('BIC nicht korrekt für Mitglied ' +
                                 combined_name + ' Error: ' + str(e))
            ...
            # Create Verwendungszweck
            d = DesignatedUse()
            verwendungszweck = d.get_designated_use(
                self._config.get_accounting_year(), b, beitragsart, combined_name)
            ...
            b = self._sepa.add_member(
                member, beitragsatz, mandat, self._config.get_accounting_date_str(), verwendungszweck)
            if b is False:
                list_of_members_wrong_sepa.append(member)
                tools.print_error('Mitglied ' + member.vorname + ' ' +
                                  member.nachname + ': SEPA Generierung schlug fehl.')
            ...
            memberTuple = (member.mitgliedsNummer, member.vorname, member.nachname,
                           datetime.datetime.strftime(
                               member.correct_eintrittsdatum, self._config.get_datetime_format()),
        ...
        if self._config.get_accounting_halfyear() == tools.BookingHalfYear.FIRST:
            ...
            booking_year_str = ' ' + \
                str(self._config.get_accounting_year()) + ' '

        tools.print_info('-------------------- Abbrechnungsjahr: ' +
                         booking_year_str + ' --------------------------')
        tools.print_info('Mitglied ohne Lastschrift:    ' +
                         str(len(list_of_members_non_debit)))
        ...
        tools.print_info('Abzubuchende Mitglieder:      ' +
                         str(nof_claimed_members) + '/' + str(len(result)))
        ...
        tools.print_info('Mitglieder von DPSG gebucht:  ' +
                         str(nof_dpsg_members))
        print("")
        tools.print_info('Verarbeitete Mandate:         ' + str(overall))
        tools.print_info('Benutzte     Mandate:         ' +
                         str(used) + '/' + str(overall))
        ...
        self.print_member_entry_this_year_as_schnupper(
            list_of_members_active_schnupper)
        ...
        # Error printing
        self.print_non_debit_members(list_of_members_non_debit)
        ...
        self.print_member_not_booked_by_dpsg(
            list_of_members_booked_here_but_not_by_dpsg)
    ...
    def print_missing_mandate_members(self, members):
        ...
        for m in members:
            ...
            tools.print_error('Mitgliedsnummer: ' +
                              str(m.mitgliedsNummer) + ' Name: ' + combinedName)
        tools.print_error(
            '-----------------------------------------------------------------------------')
        if len(members) != 0:
            tools.print_error(
                'Bitte die fehlenden Mandate in VR Networld für die Mitglieder anlegen.')
        print("")

    def print_non_debit_members(self, members):
        tools.print_info(
            '--------------------------- Überweisende ---------------------------'
        )
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
            tools.print_info('Mitgliedsnummer: ' +
                             str(m.mitgliedsNummer) + ' Name: ' + combinedName)
        tools.print_info(
            '-----------------------------------------------------------------------------')
    ...
    def print_not_used_mandate(self, mandate: VRMandat):
        tools.print_error(
            '----------------------- Nicht verwendete Mandate -----------------------')
        for m in mandate:
            combinedName = m.vorname + ' ' + m.nachname
            tools.print_error('Mandatsreferenz: ' +
                              m.mandatsreferenz + ' Name: ' + combinedName)
        tools.print_error(
            '-------------------------------------------------------------------------------')
        if len(mandate) != 0:
            tools.print_error(
                'Bitte nochmal die DPSG Nami und VR-Networld überprüfen und die nicht verwendenten Mandate entfernen.')
        print("")

    def print_member_entry_this_year_as_schnupper(self, members):
        tools.print_info(
            '-------------- Aktive Schnuppermitglieder Jahr --------------')
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
    ...
    def print_member_entry_second_half(self, members):
        tools.print_info('----------------- Mitglieder zweite Jahreshälfte ' +
                         str(self._config.get_accounting_year()) + ' ------------------')
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
            tools.print_info('Initiales Eintrittsdatum: ' + datetime.datetime.strftime(m.eintrittsdatum, self._config.get_datetime_format()) +
    ...
    def print_member_entry_new_year(self, members):
        tools.print_info('--------------------- Mitglieder im neuen Jahr ' +
                         str(self._config.get_accounting_year()+1) + ' --------------------')
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
            tools.print_info('Initiales Eintrittsdatum: ' + datetime.datetime.strftime(m.eintrittsdatum, self._config.get_datetime_format()) +
                             ' Abrechenbares Eintrittsdatum: ' + datetime.datetime.strftime(m.correct_eintrittsdatum, self._config.get_datetime_format()) +
    ...
    def print_member_booked_by_dpsg_but_not_here(self, members):
        tools.print_info(
            '-------- Mitglieder die von DPSG gebucht wurden aber nicht hier --------')
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
    ...
    def print_member_not_booked_by_dpsg(self, members):
        tools.print_info(
            '------ Mitglieder die hier gebucht wurden aber nicht von der DPSG ------')
        for m in members:
            combinedName = m.vorname + ' ' + m.nachname
            tools.print_info('Initiales Eintrittsdatum: ' + datetime.datetime.strftime(m.eintrittsdatum, self._config.get_datetime_format()) +
                             ' Abrechenbares Eintrittsdatum: ' + datetime.datetime.strftime(m.correct_eintrittsdatum, self._config.get_datetime_format()) +
    ...
    def download_invoices(self) -> PdfMemberList:
        ...
        year_start_date = datetime.date(
            self._config.get_accounting_year(), 1, 1)
        billing_sum = 0
        members_overall = []
        ...
        for n in invoices:
            if n.reDatum >= year_start_date:
                billing_sum = billing_sum + \
                    float(n.reNetto[:-4].replace(',', '.'))
                invoice = nami.invoice(groupId, n.id)
                filename = './invoices/' + str(n.id)+'.pdf'
                nami.download_invoice(
                    n.id, open_file=False, save_file=True, filename=filename)
                members = PdfConverter.convert(pdfpath=filename)
                members_overall.extend(members)

        members_overall = self.remove_unused_members(
            members_overall, self._config.get_accounting_halfyear())
